...
// Initialisation des configurations avec les variables d'environnement sécurisées const openai = new OpenAI({
    apiKey: process.env.OPENAI_API_KEY,
});

const pc = new Pinecone({
    apiKey: process.env.PINECONE_API_KEY, }); const indexName = process.env.PINECONE_INDEX_NAME || 'casablanca-knowledge';

// Route principale pour interroger l'IA (Utilisée par api.visitecasablanca.com) app.post('/api/chat', async (req, res) => {
    try {
        ...
        // Extraction du contexte trouvé (activités, entreprises, adresses à Casablanca)
